...
import (
	...
	holimanbig "github.com/holiman/big"
...
func (c *bigModExp) Run(evm *EVM, contract *Contract, readonly bool) ([]byte, error) {
	...
	var (
		base = new(holimanbig.Int).SetBytes(getData(contract.Input, 0, baseLen))
		exp  = new(holimanbig.Int).SetBytes(getData(contract.Input, baseLen, expLen))
		mod  = new(holimanbig.Int).SetBytes(getData(contract.Input, baseLen+expLen, modLen))
		v    []byte
	)
	switch {
	case mod.BitLen() == 0:
